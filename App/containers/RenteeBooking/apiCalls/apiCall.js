...
export async function getRenteeBookings(onSuccess, onFailure) {
...
export async function getOnGoing(onSuccess, onFailure) {
  const header = await getHeaders().then(data => {
    return data;
  });
  ...
  const URL = `${baseUrl}${endpoint.bookings.getmybookings}?isBooking=false&isRenter=false`;
  console.log('baseURL1 ==>', URL);
...
export async function addReview(Payload,VehicleID,onSuccess, onFailure) {
  const header = await getHeaders().then(data => {
    return data;
  });
  console.log('HEADERS ->>', header);
  ...
  console.log('Config ==>', Config);
  const URL = `${baseUrl}${vehicles}${VehicleID}${endpoint.Reviews.getAllReviewsOfVehicle}`;
...
export async function endRental(Payload,onSuccess, onFailure) {
  const header = await getHeaders().then(data => {
    return data;
  });
  console.log('HEADERS ->>', header);
  const baseUrl = Config.baseUrl.main;
  const endpoint = Config.endpoint;
  const vehicles = Config.endpoint.bookings.endrental;
